 ...
 */
public class VerifyFingerAsyncTask extends AsyncTask<String, Integer, String> {

    private Activity mActivity = null;
    private ProgressDialog pDialog = null;
    private ServiceResponce mServiceResponce = null;

    public VerifyFingerAsyncTask(Activity activity, String str) {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            entity.addPart("action","verifytermscondition");
            entity.addPart("property_id",params[0]);
            entity.addPart("tenant_id",params[1]);
            entity.addPart("usertype", params[2]);
            entity.addPart("terms", params[4]);
            File sourceFile = new File(params[3]);
            if(sourceFile.exists()) {
                // Add the data to the multi_part entity
                entity.addPart("fingerimage", sourceFile);
            } else {
                entity.addPart("fingerimage",params[3]);
            }

            httppost.setEntity((HttpEntity) entity);
            // httpPost.get
            LogConfig.logd("ServiceHandler Post Url =", "" + params[3]);
