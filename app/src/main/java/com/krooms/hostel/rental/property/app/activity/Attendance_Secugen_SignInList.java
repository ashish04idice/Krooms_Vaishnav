...
public class Attendance_Secugen_SignInList extends AppCompatActivity implements View.OnClickListener {
    ...
    GridView grid;
    public static int limtstart = 0, limtend = 4, totalpage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedStorage mSharedStorage = SharedStorage.getInstance(Attendance_Secugen_SignInList.this);
        Intent ii = getIntent();
    ...
    public void getTotaluserCount() {
        ...
        if (cc1.getCount() != 0) {
            try {
                ...
                e.printStackTrace();
                AppError.MethodLog("getTotaluserCount", "Attendance_Secugen_Signin", e.getMessage(), propertyid, versionCode);
            } finally {
                if (cc1 != null)
                    cc1.close();
            }
    ...
    public void getOfflineAttendanceCount() {
        ...
        if (cc1.getCount() != 0) {
            try {
                ...
                e.printStackTrace();
                AppError.MethodLog("getOfflineAttendanceCount", "Attendance_Secugen_Signin", e.getMessage(), propertyid, versionCode);
            } finally {
                if (cc1 != null)
                    cc1.close();
                ;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            if (devicetype.equalsIgnoreCase("Secugen")) {
                Intent i = new Intent(Attendance_Secugen_SignInList.this, Tenant_Details_Activity.class);
                i.putExtra("propertyid", propertyid);
                i.putExtra("device", devicetype);
                startActivity(i);
            } else {
                Intent i = new Intent(Attendance_Secugen_SignInList.this, Get_Attendance_Startek.class);
                i.putExtra("propertyid", propertyid);
                i.putExtra("device", devicetype);
                startActivity(i);
            }
        ...
        switch (v.getId()) {
            ...
            case R.id.click:
                try {
                    ...
                    ex.printStackTrace();
                    AppError.handleUncaughtException(ex, "Attendance_Secugen_Signin- - onClick click", propertyid, versionCode);
    ...
    private void showSignInlist() {
        ...
        if (cc1.getCount() != 0) {
            try {
                ...
                e.printStackTrace();
                AppError.MethodLog("getAbsentTenantList", "Attendance_Secugen_Signin- getAbsentTenantList ", e.getMessage(), propertyid, versionCode);
            } finally {
                if (cc1 != null)
                    cc1.close();
                if (cc != null)
    ...
    private void getVersionDevice() {
        ...
        try {
            info = manager.getPackageInfo(Attendance_Secugen_SignInList.this.getPackageName(), 0);
            versionCode = info.versionName;
            Utility.setSharedPreference(Attendance_Secugen_SignInList.this, "GetVersion", versionCode);
        } catch (Exception e) {
            AppError.handleUncaughtException(e, "Attendance_Secugen_Signin- getVersionDevice", propertyid, versionCode);
        }
    }
